...
def jira_preflight(base_url, session):
    """Diagnose the Jira connection before any search or write.

    1) /rest/api/3/myself validates authentication.
    2) /rest/api/3/project/TMOB validates access to the project.
    No issue is created or modified.
    """
    checks = []
    endpoints = [
        ("Autenticación (/rest/api/3/myself)", f"{base_url}/rest/api/3/myself"),
        ("Proyecto TMOB (/rest/api/3/project/TMOB)", f"{base_url}/rest/api/3/project/{PROJECT_KEY}"),
    ]
    for label, url in endpoints:
        try:
            r = session.get(url, timeout=30)
        except requests.RequestException as exc:
            raise RuntimeError(f"{label}: error de red: {exc}")
        if not r.ok:
            detail = r.text[:1000].replace("\n", " ")
            raise RuntimeError(
                f"{label}: HTTP {r.status_code}. Respuesta Jira: {detail}"
            )
        checks.append((label, r))
    myself = checks[0][1].json()
    project = checks[1][1].json()
    return {
    ...
    try:
        ...
        if st.button("🔎 Comprobar conexión y duplicados (sin crear)", type="secondary"):
            try:
                ...
                info = jira_preflight(base, sess)
                st.success(
                    f"Conexión Jira OK · usuario API: {info['displayName']} · "
                    f"proyecto: {info['projectKey']} — {info['projectName']}"
                )
                rows = []
                preflight_errors = []
