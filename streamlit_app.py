...
def get_allowed_options(base_url, session, field_id):
    # Current Jira Cloud create metadata endpoint.
    url = f"{base_url}/rest/api/3/issue/createmeta/{PROJECT_KEY}/issuetypes/{ISSUE_TYPE_ID}"
    last_error = None
    for attempt in range(3):
        r = session.get(url, params={"startAt": 0, "maxResults": 200}, timeout=30)
        if r.ok:
            data = r.json()
            fields = data.get("fields", [])
            field = next((x for x in fields if str(x.get("fieldId")) == field_id), None)
            if not field:
                raise RuntimeError(f"Jira no devolvió metadata del campo {field_id} para TMOB/Incidencia.")
            return field.get("allowedValues", [])
        last_error = f"HTTP {r.status_code}: {r.text[:1000]}"
        # A transient Jira 404 may return the temporary-site-unavailable payload.
        if r.status_code in (404, 429, 500, 502, 503, 504) and attempt < 2:
            time.sleep(1.5 * (attempt + 1))
            continue
        break
    raise RuntimeError(f"Error leyendo metadata de Jira tras varios intentos: {last_error}")
...
def validate_row_mappings(row, catalog):
    """Validate all catalog-backed values before any real creation."""
    errors = []
    ...
    if not option_by_name(catalog["ambito"], AMBITO_PROVISIONAL):
        errors.append(f'Ámbito no disponible: {AMBITO_PROVISIONAL}')
    if company and not resolve_empresa(catalog, company):
        errors.append(f"Empresa no encontrada en Jira: {company}")
    if group:
        try:
            resolve_group(catalog, group)
        except Exception as e:
            errors.append(str(e))
    if reason and not option_by_name(catalog["motivo"], reason):
        errors.append(f"Motivo de Pendiente no encontrado exactamente en Jira: {reason}")
    return errors
        ...
        if st.button("🔎 Comprobar conexión y duplicados (sin crear)", type="secondary"):
            try:
                ...
                for _, row in test_df.iterrows():
                    ...
                    mapping_errors = validate_row_mappings(row, catalog)
