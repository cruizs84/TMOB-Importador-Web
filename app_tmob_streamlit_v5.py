...
def resolver_empresa(v, catalog):
    ...
    if src in EMPRESA_ALIASES:
        target = EMPRESA_ALIASES[src]
        if target in catalog:
            return target, "ALIAS"
        # Si el catálogo todavía no está disponible, no inventamos.
        return None, f"ALIAS_NO_EXISTE:{target}"
    ...
    matches = [
        ...
        if normalizar(name) == normalizar(src)
    ]
